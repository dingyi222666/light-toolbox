...
function onResume()
  if pcall(function ()
      per=pm.getPackageInfo(name,PackageManager.GET_SERVICES).services
      ...
      perm_count.text="总共 "..perm_adp.getCount().." 个 Service"
      per_ls.setAdapter(perm_adp)
    end) then else
    提示 ("Service 列表加载失败，该应用可能没有 Service。")
    this.finish()
  end
end
